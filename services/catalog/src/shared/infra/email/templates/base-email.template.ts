...
export function buildOrderConfirmationEmail(params: {
  ...
  const { customerName, orderId, canteenName, total } = params;
  const content = `
    <p style="margin:0 0 16px 0;font-size:15px;line-height:1.6;color:#374151;">
      Olá, <strong>${customerName}</strong>! Recebemos o seu pedido${canteenName ? ` na cantina <strong>${canteenName}</strong>` : ""} e ele já está na fila.
    </p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:16px 0;">
      <tr>
        <td style="padding:16px;background-color:#FFF7ED;border-radius:12px;color:#7C2D12;font-size:14px;">
    ...
    <p style="margin:0;font-size:14px;color:#6B7280;">
      Você será avisado conforme o pedido avança. Bom apetite!
    </p>
  `;
  return buildBaseEmail("Pedido confirmado", content);
}
