...
import { canteensViewSchema } from "../../../projections/infra/schemas/canteen-view.schema";
...
export class ReportService {
  ...
  async adminCounts(): Promise<{ institutions: number; canteens: number }> {
    // Instituições e cantinas pertencem ao catalog-service; aqui usamos a
    // projeção local canteens_view (alimentada por eventos) para as contagens.
    const [row] = await this.drizzle.db
      .select({
        institutions: countDistinct(canteensViewSchema.institutionId),
        canteens: count(),
      })
      .from(canteensViewSchema);
    return { institutions: row.institutions, canteens: row.canteens };
  }
